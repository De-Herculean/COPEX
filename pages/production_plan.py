...
with tab4:
    ...
    if unmet.empty:
        ...
        st.caption(
            "Every product/CFA combination the model chose not to fully supply. Cost = "
            "Unmet Demand (kL) × Penalty Cost (per kL), taken directly from Exhibit D — no "
            "tier or contractual multiplier is applied on top of it."
        )

        display_cols = [
            "Product", "CFA", "Tier", "Contractual", "Unmet Demand (kL)",
            "Penalty Cost (per kL)", "Cost of Unmet Demand",
        ]
        display_cols = [c for c in display_cols if c in unmet.columns]
        view = unmet[display_cols].sort_values("Cost of Unmet Demand", ascending=False)

        st.dataframe(
            view.style.format({
                "Unmet Demand (kL)": "{:.4f}",
                "Penalty Cost (per kL)": "₹{:,.0f}",
                "Cost of Unmet Demand": "₹{:,.2f}",
            }),
            use_container_width=True, height=350,
        )
        st.download_button("⬇ Download unmet demand (CSV)", view.to_csv(index=False).encode(),
        ...
        if not contractual_unmet.empty:
            st.warning(
                ...
                f"totalling {contractual_unmet['Unmet Demand (kL)'].sum():,.1f} kL. These are "
                f"flagged for visibility — the case notes they carry consequences beyond the "
                f"modeled penalty cost, but that isn't quantified as an extra cost multiplier here."
            )
