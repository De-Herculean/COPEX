...
- **Production & distribution** is solved as a mixed-integer program (OR-Tools CBC) minimizing
  ...
  at exactly the per-kL rate in Exhibit D — no tier or contractual multiplier on top of it, since
  the case brief doesn't specify one). Tier prioritization is still honored where the case actually
  specifies it: through the tier-differentiated service levels (Exhibit F) used to size safety
  stock. Contractual status is still flagged in every relevant output for visibility, even though
  it no longer changes the cost calculation.
